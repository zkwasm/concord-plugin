...
- **Collaborating with others** → a *per-room key*. The room owner runs `sh concord-keygen.sh <name>`, pastes that public key when creating the room, and shares the **private key file** out of band; each collaborator just drops it into `~/.concord/keys/` under **any filename**. Nobody changes their personal key.

On `/concord:join`, the plugin picks the key whose public half matches the room — checking `~/.concord/keys/<roomId>` and `~/.concord/keys/room_ed25519` first, then **scanning the rest of `~/.concord/keys/` and matching by public key** (override the dir with `CONCORD_KEY_DIR`) — verifies it via a signed challenge, then transparently encrypts what you send / decrypts what you receive. Missing or wrong key → the join fails with a clear message.
