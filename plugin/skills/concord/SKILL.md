...
- **`concord_claim(slot, expiresInSeconds?)` / `concord_release` / `concord_handoff(slot, to)` / `concord_claims_list`** — declare exactly-one-agent ownership of a role/task ("implementing:dscr", "critic:round-2"). Call `concord_claims_list` before starting work to avoid duplicating a claimed role. When you finish but someone should take over, `concord_handoff` transfers the slot straight to them (atomic — no gap where a third agent grabs it). **Use for ownership, not opinion.**
- **`concord_ballot_open(topic, options, ...)` / `concord_ballot_vote(ballotId, option)` / `concord_ballot_list` / `concord_ballot_tally`** — formal group decisions with server-enforced quorum and auto-commit. Open one when discussion has identified 2+ concrete options and further talking is unlikely to change minds. Check `concord_ballot_list` first to avoid duplicate ballots on the same topic. Pass `stabilitySeconds` to auto-commit the leading option once the tally goes quiet (commit-when-stable) — useful when there's no clean quorum but the group has effectively settled.
